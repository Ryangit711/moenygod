...
## Forex Clock & Session Tools – Quick Reference

> **Purpose**: A curated list of free, web‑based tools that show when the major FX sessions are active, help you spot overlapping windows, and give you a quick glance at market‑open/close times. Use them together with the liquidity‑flow framework in **08-forex-trading-the-purest-flow.md** and the weekly dashboard in **13-weekly-flow-tapping-operating-system.md**.
...
| 2 | [https://www.forex.academy/navigating-time-zones-the-best-times-for-international-forex-trading/](https://www.forex.academy/navigating-time-zones-the-best-times-for-international-forex-trading/) | Article + tables that break down each session’s typical volatility, major economic releases, and best pairs to trade per session. | Planning: which pairs are most active during a given session; helps align your watch‑list with the session’s flow. |
...
| 4 | [https://www.forexfactory.com/thread/102905-world-forex-market-times-desktop-tool-free](https://www.forexfactory.com/thread/102905-world-forex-market-times-desktop-tool-free) | Free desktop widget (Windows/macOS/Linux) that displays session times, overlaps, and can be pinned to your taskbar/menu bar. | Persistent overlay while you work; no need to keep a browser tab open. |
...
> **Note**: The original list included an incomplete “h…” link. If you have a specific URL in mind (e.g., a heat‑map or liquidity‑visualiser), replace the placeholder above with the correct link.
...
| forex.academy article | ❌ (static images) | ✅ Tables with GMT offsets | ✅ Lists best pairs per session | ❌ | Economic‑calendar notes, volatility stats |
| market24hclock.com | ✅ Circular 24‑h clock | ✅ Shows current UTC/local | ✅ Color‑coded overlap bands | ❌ | Simple, low‑resource, embeddable |
| ForexFactory desktop widget | ✅ Mini‑map + countdown | ✅ Local time display | ✅ Highlights overlap periods | ✅ (customizable pop‑up/alarm) | Can be set to start with OS |
...
|-----|----------------|--------------------------|
| **No consolidated 24‑hour overlap matrix for CAD‑pairs** | CAD liquidity is driven by both US and Asian sessions; a single table showing “CAD‑pair activity %” per hour would let you pick the optimal window without checking multiple charts. | Create a simple heat‑map (hour × pair) using historical average volume; embed as a PNG or interactive table in this doc. |
| **No real‑time liquidity/heat‑map** | Knowing *when* a session is open is useful, but knowing *where* the liquidity sits (e.g., clusters of stops, order‑book depth) gives an edge. | Integrate a free order‑book snapshot API (e.g., Binance for BTC/USD, or FXCM’s free depth feed) for major pairs; overlay as a semi‑transparent heat‑map on the session clock. |
| **No economic‑calendar overlay** | News releases cause‑session sync** – you need to know if a high‑impact report coincides with a session open/close (liquidity vacuum or spike). | Pull data from Forex Factory or Investing.com calendar (CSV/JSON) and mark event times on the session clock with icons. |
...
1. **Morning Prep (5 min)**  
   ...
   - Check the **forex.academy** table for the day’s highlighted pairs and any major economic releases (note the time).  

2. **During Trading**  
   - Keep the **ForexFactory desktop widget** pinned; it will flash when you enter/exit a high‑liquidity overlap.  
   ...
   - Before each trade, ask:  
     *Is the price currently inside a high‑liquidity overlap?*  
     *Is there a high‑impact news event within the next 15 min?*  
     *Does the session‑specific bias (from the forex.academy article) agree with your setup?*  
...
4. **Long‑Term Upgrade (Optional)**  
   - Build or source a **liquidity‑heat‑map** that updates every minute using order‑book data from a ECN broker (e.g., IC Markets, Pepperstone).  
...
- **[08-forex-trading-the-purest-flow.md](./08-forex-trading-the-purest-flow.md)** – Six forces that move FX, Canadian trader universe, carry trade, news fade, London fix.  
...
- **[13-weekly-flow-tapping-operating-system.md](./13-weekly-flow-tapping-operating-system.md)** – Weekly liquidity dashboard (BoC balance sheet, EIA inventories, COT, etc.) to see where the macro flow is heading.  
...
[ ] Skim forex.academy article – note today’s highlighted pairs & news times
[ ] Keep ForexFactory widget pinned (set alarm for London/NY overlap)
[ ] If using TradingView, add the community session‑lines script
[ ] Before each trade: ask “Am I in a high‑liquidity overlap? Any news soon?”
